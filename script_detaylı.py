...
class GoProVideoTrack(VideoStreamTrack):
    ...
    def _get_frame(self):
        ...
        if not ret:
            return None
        # Optimize by resizing and encoding directly for WebRTC
        frame = cv2.resize(frame, (640, 360))  # Reduce resolution for lower latency
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2YUV_I420)
        return frame

def create_peer_connection():
    pc = RTCPeerConnection()
    # Lower buffer size to reduce latency
    pc.setConfiguration({"iceCandidatePoolSize": 4})
    return pc
...
async def main(gopro_url):
    ...
    while True:
        await asyncio.sleep(0.01)  # Reduce sleep time for responsiveness
